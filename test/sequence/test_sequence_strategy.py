...
class TestSequenceStrategy(unittest.TestCase):
    ...
    def test_binary_sequence(self):
        ...
        seq = NArySequence(values, self.always_true, 2)
        assert seq.next() == 0
        assert seq.next() == 99
        assert seq.next() == 50
    ...
    def test_binary_sequence_ending_only_even_available(self):
        values = list(range(100))
        seq = NArySequence(values, self.only_even, 2)
    ...
    def test_3ary_sequence(self):
        values = list(range(100))
        seq = NArySequence(values, self.always_true, 3)
        assert seq.next() == 0
        assert seq.next() == 99
        assert seq.next() == 34
        assert seq.next() == 67
    ...
    def test_3nary_sequence_ending_only_even_available(self):
        values = list(range(100))
        seq = NArySequence(values, self.only_even, 3)
        outputted_values = set()
        for _ in range(50):
            n = seq.next()
            assert n % 2 == 0
    ...
    def test_4ary_sequence(self):
        values = list(range(100))
        seq = NArySequence(values, self.always_true, 4)
        assert seq.next() == 0
        assert seq.next() == 99
        assert seq.next() == 26
        assert seq.next() == 51
    ...
    def test_4nary_sequence_ending_only_even_available(self):
        values = list(range(100))
        seq = NArySequence(values, self.only_even, 4)
        outputted_values = set()
        for _ in range(50):
            n = seq.next()
            assert n % 2 == 0
    ...
    def test_limit(self):
        values = list(range(100))
        seq = NArySequence(values, self.always_true, 2, limit=37)
        for _ in range(37):
            seq.next()
        self.assertRaises(SequenceFinished, seq.next)
